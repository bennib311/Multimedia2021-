...
static void init_vertex_data(user_data_t* user_data, int edges)
{
	// Polygon data:
	vertex_data_t vertex_data[edges]; 
	float rel;

	for (int i = 0; i < edges; i++) { 

		// Setting the x,y coordinates for the edges
		vertex_data[i].position[0] = (0.8 * cos(2 * M_PI * i / edges)); 
		vertex_data[i].position[1] = (0.8 * sin(2 * M_PI * i / edges));
		vertex_data[i].position[2] = 0;

		rel = (float)i / (float)edges;

		// Setting the RGB-values to create a full color circle
		if (rel <= 0.333) {
			vertex_data[i].color[0] = 255 - rel * 3 * 255;
			vertex_data[i].color[1] = rel * 3 * 255;
			vertex_data[i].color[2] = 0;
		} else if (rel > 0.333 && rel <= 0.666) {
			vertex_data[i].color[0] = 0;
			vertex_data[i].color[1] = 255 - 255 * (rel * 3 - 1);
			vertex_data[i].color[2] = 255 * (rel * 3 - 1);
		} else if (rel > 0.666) {
			vertex_data[i].color[0] = 255 * (rel * 3 - 2);
			vertex_data[i].color[1] = 0;
			vertex_data[i].color[2] = 255 - 255 * (rel * 3 - 2);
		}
	}
	...
	glBufferData(GL_ARRAY_BUFFER, edges * sizeof(vertex_data_t), (const GLvoid*)vertex_data, GL_STATIC_DRAW);
...
void init_gl(GLFWwindow* window, int edges)
{
	...
	init_vertex_data(user_data, edges);
...
void draw_gl(GLFWwindow* window, int edges)
{
